...
#define OLED_W_SCL(x) HAL_GPIO_WritePin(GPIOC, SCL_Pin, (GPIO_PinState)(x))
#define OLED_W_SDA(x) HAL_GPIO_WritePin(GPIOC, SDA_Pin, (GPIO_PinState)(x))
...
void OLED_I2C_Init(void)
{
	...
	// 关闭入侵检测功能，将PC13引脚设置为普通GPIO引脚
	HAL_PWR_EnableBkUpAccess();      // 允许修改RTC和后备寄存器
	// 关闭外部低速时钟（LSE）
	...
	HAL_PWR_DisableBkUpAccess();     // 禁止修改RTC和后备寄存器
	//HAL_RTCEx_BKUPWrite(&hrtc, RTC_BKP_DR0, 0); // 可以使用RTC备份域存储一些关键数据，如设备ID等
